    ...
    { headeritself && <div className="headerSection">
             <div>
                ...
                {showlist && <><div className="blackOut"></div><ul onMouseLeave={stay ? () => '':() => { setShow(false); setShowlist(false)}} className='headerList'>
                                <li className="headerName">{auth && <img src={imagefield !== null & imagefield !== 'undefined' ? 'https://dudichatappbackend.herokuapp.com' + imagefield : DefaultImage} alt="UserImage" className="profileimageheader"/>}<div className="profileimageheadername">{nameis}</div></li>
                                ...
                                <li style={{color:"red",marginLeft:"-10%"}}>{auth ? <small>{datacreated}</small> : <small>No time record</small>}</li>
                                <li style={{color:"cyan",marginLeft:"-10%"}}>{star ? "You are star" :<small>You have no star</small>}</li>
                            </ul>
                            </>
                    }
            </div>
